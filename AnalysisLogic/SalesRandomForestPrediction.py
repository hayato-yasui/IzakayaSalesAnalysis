...
class SalesRandomForestPrediction:
    ...
    def _create_decision_tree(self):
        ...
        X, y = self._create_prd_and_obj_valiables(self.df_preproc, '客単価高フラグ')
        X = pd.get_dummies(X)
        ...
        dot_data = tree.export_graphviz(estimators[0],  # 決定木オブジェクトを一つ指定する
                                        ...
                                        # class_names=X.target_names,  # これを指定しないとチャート上で分類名が表示されない
...
if __name__ == '__main__':
    srfp = SalesRandomForestPrediction()
    srfp.execute()
